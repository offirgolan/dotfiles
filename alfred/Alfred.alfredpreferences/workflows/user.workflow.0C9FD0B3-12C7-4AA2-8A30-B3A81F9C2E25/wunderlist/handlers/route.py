    ...
    if handler:
        if '--commit' in args:
            ...
            if workflow().update_available:
                ...
                if update_data.get('version') != '0.6.0':
                    workflow().add_item('An update is available!', 'Update the Wunderlist workflow from version 0.6.0 to %s' % update_data.get('version'), arg='-about update', valid=True, icon=icons.DOWNLOAD)
